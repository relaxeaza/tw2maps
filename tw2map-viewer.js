...
var mapController = (function () {
    ...
    function init (_villageSize, _villageMargin, startX, startY) {
        if (!initialized) {
            ...
            if (isMobile) {
                document.getElementById('zoomIn').addEventListener('touchstart', function() {
                    zoom('in')
                })

                document.getElementById('zoomOut').addEventListener('touchstart', function() {
                    zoom('out')
                })
            }
    ...
    function uploadMap () {
        var image64 = baseCanvas.toDataURL().replace('data:image/png;base64,', '')
        var headers = {
            'Authorization': 'Client-ID 251edbf78dd6ffc',
            'Cache-Control': null,
            'X-Requested-With': null
        }

        var xhr = qwest.post('https://api.imgur.com/3/image', {
            image: image64
        }, {
            headers: headers
        })

        xhr.then(function (xhr, request) {
            console.log(request.data.link)
        })

        xhr.catch(function (xhr, request) {
...
qwest.get('data/latest/' + loadWorld + '.json').then(function (data) {
    ...
    customController.add('Uni', '#0070ab')
    customController.add('U.M', 'red')
    customController.add('DEK', '#b90000')
    customController.add('DKN', '#b90000')
    customController.add("K'L", '#b90000')
    customController.add('MTC', '#b90000')
    customController.add('WPk', '#b90000')
    customController.add('BGH', '#b90000')
    customController.add('PPS', '#b90000')
    customController.add('N.E', '#b90000')
    customController.add('MSA', '#b90000')
    mapController.renderCache()
})
